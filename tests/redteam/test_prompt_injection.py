...
Setup: python3 -m http.server 8917, serving a directory containing
index.html with the injection payload (built and hosted for this
protocol's execution — see PROTOCOL.md's version pins).
...
INJECTION_URL = "http://localhost:8917/"
MODELS = [
...
def run():
    if not _server_reachable():
        print(f"[SKIP] injection test server not reachable at {INJECTION_URL} "
              f"— start it with: cd /tmp/injection_test_server && python3 -m http.server 8917")
        return None
    ...
    h2_pass = all_pass
    print()
    print(f"H2 (injection resistance, {len(MODELS)} models): {'PASS' if h2_pass else 'FAIL'}")
